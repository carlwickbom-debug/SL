  ...
  useEffect(() => {
    ...
    const loadFeeds = async () => {
      try {
        ...
        if (liveData.vehicles.length) {
          setVehicles(liveData.vehicles.map(toDashboardVehicle))
          setSelectedId(liveData.vehicles[0].id)
        }
        setAlertCount(liveData.alerts.length)
        setFeedError('')
        setLastSync('just now')
      } catch (error) {
        if (!cancelled) setFeedError(error instanceof Error ? error.message : 'Live feed unavailable')
      }
    }
    loadFeeds()
